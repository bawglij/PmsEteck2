...
$.extend($.fn, {
	...
	validate: function( options ) {
		...
		if (!this.length) {
			options && options.debug && window.console && console.warn( "Niets geselecteerd, kan niet valideren, geeft niets terug" );
			return;
		}
	...
	prototype: {
		...
		check: function( element ) {
			...
			for (var method in rules) {
				...
				try {
					...
					this.settings.debug && window.console && console.log("Er is iets fout gegaan bij het controleren van " + element.id
						 + ", controleer de '" + rule.method + "' methode", e);
					throw e;
				}
			}
		...
		defaultMessage: function( element, method) {
			return this.findDefined(
				...
				"<strong>Opgelet: Er is geen melding voor " + element.name + "</strong>"
			);
		},
		...
		showLabel: function(element, message) {
			...
			if ( label.length ) {
				...
				label.attr("gegenereerd") && label.html(message);
			} else {
				// create label
